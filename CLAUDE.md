...
- `clipboard_history.rs` — in-memory clipboard history. Polls `NSPasteboard.changeCount` (macOS has no pasteboard-change notification) and records plain text into a 10-entry ring, newest first, re-copies promoted rather than duplicated. Emits `clipboard:new`. Skips any write carrying `org.nspasteboard.ConcealedType` / `TransientType` / `AutoGeneratedType` (the marker password managers set) — that check is the difference between a utility and a liability. Nothing is persisted; the ring dies with the process.
...
- `context/clipboard-history.tsx` + `components/clipboard-history-button.tsx` — the titlebar clipboard dropdown. `applyClip` mirrors the Rust ring's promote-on-recopy so the UI stays in step between events without a round trip. Recording can be turned off; the choice is pushed to the backend on mount, before anything can be captured.
