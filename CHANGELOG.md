...
- **Clipboard history in the titlebar** ([#79](https://github.com/willywg/klaudio-panels/issues/79),
  ...
  notification bell keeps the last 10 things that hit the pasteboard; click one
  to put it back.

  It watches the **system pasteboard**, not the terminal, which is what makes
  it source-agnostic for free — `pbcopy` from Claude's Bash tool, ⌘C in
  xterm.js and a copy from Safari all land in the same place. macOS has no
  pasteboard-change notification, so it polls `NSPasteboard.changeCount`, a
  monotonic integer bumped on every write: an integer read per tick, and it
  correctly catches copying the same text twice, which content-diffing cannot
  tell apart from nothing happening.

  Writes marked `org.nspasteboard.ConcealedType` (the convention password
  managers use), `TransientType` or `AutoGeneratedType` are never recorded —
  without that check the dropdown would quietly hold your 1Password output.
  Nothing is persisted to disk; the ring dies with the process. Recording can
  be turned off from the dropdown, and turning it back on does not backfill.
